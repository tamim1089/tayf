...
| **Holographic SLM / computer-generated holography** | Real, mature field (Gerchberg-Saxton, multiplane CGH e.g. 2205.07030). Iterative hologram synthesis: 57s per 8-plane frame on an RTX 3070 — nowhere near real-time; deep-learning "tensor holography" gets to ~1.5s | Full parallax, coherent-light speckle artifacts, narrow viewing angle (a few degrees at useful resolution per 2203.06784) | Real but currently too slow and too narrow-angle for a moving conversational avatar |
| **Light-field / retroreflective panel (Looking Glass-class, AIP)** | Commercial products exist today (Looking Glass, Sony ELF-SR2) | Not free space — image is bound to a physical panel — but glasses-free, real depth cues, works today, fits a compact enclosure | **Not what the user asked for, but the only mechanism that is actually buildable, safe, and demo-able by Sep 13** |
