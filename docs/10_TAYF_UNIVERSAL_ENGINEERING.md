...
|---|---|---|
| FR-1 | **Capture** body pose, facial expression and hand articulation within a user-set volume, without room-scale infrastructure | §5 |
| FR-2 | **Enrol** a persistent personalised avatar once, offline, separating identity from dynamic state | §5 |
| FR-3 | **Encode** each frame to a compact driving state and quantise it | §3.4, §6 |
| FR-4 | **Transport** within the latency budget, degrading gracefully | §6 |
| FR-5 | **Reconstruct** by animating the enrolled avatar from received state | §5, §8 |
| FR-6 | **Emit** as a real image in open air, no screen carrying the picture | §4 |
| FR-7 | **Register** a spatial frame so the image holds a stable position and scale | §8 |
| FR-8 | **Track** the observer well enough to drive angular allocation | §8 |
| FR-9 | **Be symmetric** — every device does FR-1…FR-8 in both directions at once | §1 |

**Hard constraints** (violating one means it is not TAYF): free-space output · no wearables · life-size · in the viewer's space · photoreal · symmetric endpoints · self-contained · any ordinary room · eye-safe · ≤150 ms one-way.

**Adjustable parameters** (negotiable under evidence): aperture size · simultaneous observers · angular coverage · apparent subject (face → full body) · frame rate · viewing distance · power delivery. **Aperture size is the parameter to spend first** — it is set by §2's laws, not by taste, and every product form in §3 is one choice of it.
...
![The two aperture modes](../models/svg/aperture_modes.svg)

*Both modes are legitimate. Naming which one you mean is mandatory — conflating them is the easiest technical error in this project.*
...
![Presence is an angle](../models/svg/angular_presence.svg)
...
*The mechanism, drawn from the geometry rather than reproduced from any paper. Three static elements; the image forms in open air with nothing at its location.*

![Optical efficiency cascade](../models/svg/efficiency_cascade.svg)

*Where the light goes. The retroreflector return efficiency η_RR is the one term nobody has measured, and it gates every panel-power figure in this document.*
