...
|---|---|---|
| **`13_THE_ROOM.md`** | **LIVE** | The product: 360° free-space telepresence room, `N = 2πz/D`, 15–19 engines, viewers at 1.3–1.8 m. `thedream.md` rules 4/6/8 deliberately suspended. Carries three boxed corrections (§6, §7, §11) — read them, they are the useful part. **Doc 17 corrects its elevation coverage and generalises its tiling law.** |
| **`17_THE_MEETING_ROOM.md`** | **LIVE** | Seated people, a rectangular plan, mechanical steering. Generalises the tiling law to `N = perimeter/D`. Derives the elevation relation doc 13 never wrote, which shows a standing full-body image loses everything below mid-thigh to geometry. Bust framing + two rows facing cuts band area 52%. Costs the rectangle at 4 engines per metre. |
| **`18_THE_FIELDS_WE_STAND_ON.md`** | **LIVE** | Every scientific field this project draws on and exactly what it takes from each, with the governing relation. Includes the niche connections — isoperimetric inequality sets the engine count, Liouville's theorem is why étendue is a law, a cylindrical room focuses sound on the speaker — and the list of fields deliberately *not* used. |
